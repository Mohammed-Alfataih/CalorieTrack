  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (Array.isArray(content)) {
        ...
        // Use AI to describe image first
        const visionResult = await env.AI.run(
          "@cf/llava-hf/llava-1.5-7b-hf",
          {
            image: [
              ...Uint8Array.from(
                atob(imagePart.image_base64),
                (c) => c.charCodeAt(0)
              ),
            ],
            prompt:
              "What food is in this image? List all visible foods with approximate portion sizes.",
            max_tokens: 256,
          }
        );
      ...
      const calorieResult = await env.AI.run(
...
- confidence: high if common food, medium if ambiguous, low if unclear`,
      ...
      const translateResult = await env.AI.run(
        "@cf/meta/llama-3.1-8b-instruct",
        {
          messages: [
            {
              role: "system",
              content:
                "Translate the following food name to Arabic. Reply with ONLY the Arabic translation, nothing else.",
            },
            {
              role: "user",
              content: parsed.foodName,
            },
          ],
          max_tokens: 50,
        }
